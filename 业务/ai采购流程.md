...
4. 小王写表并回读校验后，必须发飞书给阿文同步本次结果；消息列出“供应商单据 -> 对应采购单号”，并说明写入条数、合计金额、异常/换算口径、是否已写入 WorkBuddy。
...
- 写表后必须飞书同步阿文，不能只在本地完成。飞书格式优先用清单：标题 `小王供应商单据对应采购单号`，逐行 `供应商 | 单据号/物料 | 对应: MCG...`，末尾写合计、WorkBuddy 写入状态、尚未/已交影刀执行状态；发送失败必须显式回报。
