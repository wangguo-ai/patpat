...
当前状态：小王已补齐“自动登录/进入图灵采购单管理并下载待下单、待供应商接单数据”“识别供应商微信映射表中的无需发送微信供应商”“待下单页面按采购单勾选、空采购员转派王国钦、批量下单”；三表生成、微信映射、到货处理文件可复用。微信发送、采购完成、打印标签、交接小吴尚未完整实现。
...
| Agent | 脚本 | 处理方式 |
|------|------|------|
| 小王 | `D:\报销工作台\03_脚本\xiaowang_agent.py` | 已具备采购单下载入口、待下单批量下单入口、空采购员转派入口；后续继续补微信发送、采购完成等业务动作 |
...
仍待开发：
1. 自动发送微信给供应商。
2. 根据供应商单据图片识别采购单、数量、价格。
3. 在待供应商接单或待收货页面点击“采购完成”。
4. 自动录入实际采购数量、实际采购单价。
5. 提交并打印标签。
6. 将采购完成单号写入小吴任务表。
...
|------|------|------|
| 阿文调度 | 0% | 尚无统一调度和异常回流 |
| 小王采购链路 | 约 45% | 采购单下载、三表生成、微信映射、无需发送微信识别、待下单批量下单、空采购员转派、到货处理文件可用；微信发送、采购完成页面操作仍未实现 |
