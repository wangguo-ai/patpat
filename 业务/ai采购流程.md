...
3. 录入实际采购数量和实际采购单价。
4. 点击提交。
5. 提交并尝试打印标签；标签是否实际打印成功需要单独验证。
6. 把采购完成的采购单同步给小吴，作为待入库任务。

当前状态：小王已补齐“自动登录/进入图灵采购单管理并下载待下单、待供应商接单数据”“识别供应商微信映射表中的无需发送微信供应商”“读取三表 `待发供应商信息` 中的发送状态/发送时间，已发送后继续图灵下单”“待下单页面按采购单勾选、空采购员转派王国钦、批量下单”“供应商单据扫描识别入口”“待收货优先的采购完成入口”“成功后写入小吴任务表”。2026-06-22 已用宝年丰布业 `BNF2606180279` 测试 `MCG202606180018`、`MCG202606180021`，采购完成提交与写入小吴任务表跑通；标签打印仅确认脚本会尝试点击，尚未确认弹窗、最终打印动作或打印机出纸。三表生成、微信映射、到货处理文件可复用。微信发送由影刀负责，小王不直接点击微信发送。供应商单据 OCR 不完整时进入 `待Agent识别`，不自动提交。采购完成批量能力可用，但只处理能唯一确定 `采购单号 / 实际采购数量 / 实际采购单价 / 来源单据` 且金额、单位、单价无矛盾的记录；怡丰、明达、优粤等存在数量/单位/单号歧义的记录先排除在自动批量外，等待人工确认。
...
| 小王供应商单据识别入口 | `D:\报销工作台\03_脚本\xiaowang_agent.py --scan-supplier-docs --dry` | 小王 | 扫描 `D:\报销工作台\01_供应商单据`，优先调用 Umi-OCR；识别采购单、实际数量、实际单价；输出 `xiaowang_supplier_doc_scan.json` | 识别不全则标记 `待Agent识别`，不提交图灵 |
| 小王采购完成入口 | `D:\报销工作台\03_脚本\xiaowang_agent.py --complete-purchases` | 小王 | 先在“待收货”搜索采购单，找不到再去“待供应商接单”；勾选后点击采购完成，录入实际采购数量/单价，提交并尝试打印标签，成功后写小吴任务表 | 只处理识别成功且无歧义记录；真实执行前必须先 `--dry`；标签打印未单独验证前不能汇报为已打印成功 |
...
4. 采购员为空时自动转派“王国钦”。
5. 供应商单据图片扫描入口：优先 Umi-OCR，识别不全转 `待Agent识别`。
