...
> 小王回货签收优先速读：[[业务/小王回货签收速读卡|小王回货签收速读卡]]
...
4. 小王写表并回读校验后，必须发飞书给阿文同步本次结果；固定发送到飞书会话 `oc_4070d243f4bf5f6c5c2a4fd5f2d11c32`，发送时使用 `lark-cli.cmd --profile cli_aabb9de267395cde`，不要发到 Hermes/飞书 CLI 默认私聊会话。消息列出“供应商单据 -> 对应采购单号”，并说明写入条数、合计金额、异常/换算口径、是否已写入 WorkBuddy。
5. 影刀读取入库任务表，进入图灵待供应商接单或待收货页面点击“采购完成”。
6. 影刀录入实际采购数量和实际采购单价，点击提交，并执行打印标签。
7. 影刀回写系统执行结果或由阿文/小王根据结果更新任务状态。
...
- 怡丰布行做回货签收时，要用供应商电子单总金额反核每条实际单价。怡丰部分物料会比图灵参考价或单据行面价优惠 10 元/米；例如 2026-06-29 `XS-260626513` 应按总价 90 元执行：52# 驼色 20 元/米、137# 彩蓝 25 元/米、156# 芥黄 25 元/米、10# 彩蓝 20 元/米。不能只按参考价写 30/35，写表前必须确认合计金额对得上供应商单据总价。

2026-07-06 防呆补充：
- 回货签收写表前必须做“三个总额校验”：供应商单据总额、待写入明细合计、写入后 WorkBuddy 回读汇总。三者不一致时不能交给影刀执行。
- 怡丰优先看红字/底部单据总额，不要只加行面价。`XS-260703417` 行面价合计 285，但单据总额 205，正确做法是按总额倒核，本次按每米优惠 10 元修成 205。
- 宝年丰、恒业、华讯昌达等按公斤/KG开单但采购单按米入库的，必须用供应商金额倒核米价，不能抄采购单参考价，也不能把公斤数量直接写成本次入库米数。
- 同一供应商同一单据跨多图、多页或多色号时，先合并核总，再按 MCG 拆明细；同色号对应多个 MCG 时按采购单需求数量拆分，并在备注写明拆分依据。
- 最终回读必须按供应商汇总检查高风险项：怡丰是否等于供应商总额、宝年丰是否等于供应商总额、整批是否无遗漏/无重复/无 `?` 乱码。
- 写表后必须飞书同步阿文，不能只在本地完成。固定发送到会话 `oc_4070d243f4bf5f6c5c2a4fd5f2d11c32`，发送时使用 `lark-cli.cmd --profile cli_aabb9de267395cde`；不要发到 Hermes/飞书 CLI 默认私聊会话。飞书格式优先用清单：标题 `小王供应商单据对应采购单号`，逐行 `供应商 | 单据号/物料 | 对应: MCG...`，末尾写合计、WorkBuddy 写入状态、尚未/已交影刀执行状态；发送失败必须显式回报。
- 飞书长消息必须用 UTF-8 脚本或参数数组发送，禁止把整段中文清单直接塞进 PowerShell 一行命令；否则可能只发出标题、正文丢失。长清单优先拆成多条短消息逐条发送，避免 `--text` 多行正文被飞书/CLI 截断。发送后要确认飞书消息正文完整，不完整必须补发。

2026-07-13 防呆补充：
- 回货签收逐张识别供应商单据时，先读抬头、地址、电话、物料名，再匹配采购导出；不能因为相邻图片或上一张图片是某供应商，就把下一张手写单沿用同一供应商。2026-07-13 曾把广新手写米白 2 米误判为鑫悦，正确应为广新布行。
- 同物料同色号多张 MCG 时，先查 WorkBuddy 已写入的 MCG，再把新增供应商单据数量拆给尚未写入的 MCG，避免漏录或重复。广新 75D双面佳积布米白/148# 最终口径：7/10 单据 1 米对应 `MCG202607090044`；手写米白 2 米 20 元对应 `MCG202607090005`、`MCG202607090021`。
- 用户纠错后，必须同时修正 WorkBuddy 和飞书口径：备份原表、补写/修正、回读新增 MCG、检查无重复/无遗漏/无 `?` 乱码，再补发飞书更正。仅在聊天里承认错误但不修表，不算完成。
- 飞书结果必须按“供应商单据 -> 对应采购单号”列出，不要只发“写入 X 条、金额 Y 元”的执行汇总。推荐格式：`供应商 | 单据号/物料颜色 | 对应：MCG...`；多余单据要写明“多余单据，本次不录入”。
- 2026-07-13 批次最终写入 WorkBuddy 为 36 张采购单、890 元；其中怡丰 `XS-260710407`（42#深灰 0.5 米、13#米杏 0.5 米，26 元）为多余单据，不录入。
