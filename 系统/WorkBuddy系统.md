...
- **工具能力**：采购单下载、读取三表发送状态后待下单批量下单、空采购员转派王国钦、供应商单据识别、采购完成、打印标签尝试、写小吴任务表、版料采购单生成三表、待发供应商信息、待补联系人、无需发送微信供应商识别。
...
| 脚本 | 归属 | 功能 |
|------|------|------|
| `D:\报销工作台\03_脚本\xiaowang_agent.py` | 小王 | 采购员 Agent 主入口；下载采购单；读取三表发送状态后待下单批量下单；供应商单据识别；待收货优先采购完成；打印标签尝试；写小吴任务表 |
...
- 当前状态：数据底座和小李报销链路较完整；小王已具备采购单下载、无需发送微信识别、读取三表发送状态后待下单批量下单、空采购员转派、供应商单据扫描、采购完成入口、小吴任务表写入能力；微信发送由影刀负责，小吴齐套出库动作、阿文统一调度仍需开发。
