...
- **工具能力**：采购单下载、版料采购单生成三表、待发供应商信息、待补联系人、无需发送微信供应商识别。
...
| 脚本 | 归属 | 功能 |
|------|------|------|
| `D:\报销工作台\03_脚本\xiaowang_agent.py` | 小王 | 采购员 Agent 主入口；自动下载待下单、待供应商接单并合并当天采购单 |
...
- 当前状态：数据底座和小李报销链路较完整；小王已具备采购单下载和无需发送微信识别能力；小王微信发送/下单/采购完成页面动作、小吴齐套出库动作、阿文统一调度仍需开发。
